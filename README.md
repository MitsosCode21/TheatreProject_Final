...
## 8. Ρύθμιση Auth0 για Σύνδεση μέσω Google (Sign in with Google)

Για να λειτουργεί σωστά η σύνδεση μέσω Google, πρέπει να καταχωρήσετε την IP του υπολογιστή σας στις ρυθμίσεις της εφαρμογής στο **Auth0 Dashboard**.

### Βήμα 1: Βρείτε την IP σας
Εκτελέστε σε τερματικό (Windows: `cmd`, Mac/Linux: `terminal`):
```bash
ipconfig
```
Αντιγράψτε την **IPv4 Address** (π.χ. `192.168.X.X`) από την ενότητα Wi-Fi.

### Βήμα 2: Auth0 Dashboard → Theatre app → Settings

1. Συνδεθείτε στο [manage.auth0.com](https://manage.auth0.com)
2. Πηγαίνετε **Applications → Theatre app → Settings**
3. Κατεβείτε στην ενότητα **"Application URIs"**

### Βήμα 3: Προσθέστε την IP σας

Στα παρακάτω πεδία, προσθέστε το URL με τη δική σας IP (διατηρείτε και τα υπόλοιπα που υπάρχουν ήδη):

**Allowed Callback URLs** — προσθέστε:
```
exp://ΝΕΑ_IP:8081
```

**Allowed Logout URLs** — προσθέστε:
```
exp://ΝΕΑ_IP:8081
```

**Παράδειγμα:** Αν η IP σας είναι `192.168.1.10`, προσθέτετε `exp://192.168.1.10:8081` και στα δύο πεδία.

4. Πατήστε **"Save Changes"** στο κάτω μέρος της σελίδας.

> **⚠️ Σημείωση:** Αν αλλάξετε δίκτυο Wi-Fi, η IP αλλάζει. Επαναλάβετε τη διαδικασία με τη νέα IP για να συνεχίσει να δουλεύει το Google login.
